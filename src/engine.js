...
export class MNAnimat3DEngine extends EventTarget {
  ...
  onPointerUp(event) {
    ...
    const meshes = [];
    this.editorRoot.traverse(object => { if (object.isMesh || object.isPoints || object.isLine) meshes.push(object); });
    this.scene.children.forEach(child => {
      if (child !== this.editorRoot && child !== this.grid && !child.isLight && !child.isCamera) {
        child.traverse(object => { if (object.isMesh || object.isPoints || object.isLine) meshes.push(object); });
      }
    });
    const hit = this.raycaster.intersectObjects(meshes, false)[0];
    if (!hit) { this.select(null); return; }
    let object = hit.object;
    while (object && object !== this.editorRoot && object !== this.scene && !object.userData.editable) {
